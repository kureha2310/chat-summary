...
収集方法は3つあります。用途に合わせて使い分けてください。

### 収集方法 1：個別ピックアップ

気になるメッセージに `:pushpin:` や `:bookmark:` などのリアクションを付けると、Bot がラベル付きでバッファに蓄積します。
最後に `:checkered_flag:` を押すと、そのスレッドのバッファを OpenAI で整理して Notion に保存します。

### 収集方法 2：スレッド一括

スレッド内のメッセージを選別せず全部まとめたい場合は、`:spiral_notepad:` を付けるとスレッド全件を一括収集します。
あとは `:checkered_flag:` でまとめ実行するだけです。

### 収集方法 3：スレッド横断（チャンネル全体）

複数スレッドにまたがってリアクションを付けておき、`:dart:` を押すとチャンネル内の全バッファをまとめて Notion に保存します。
スレッドの垣根を超えて横断的にまとめたいときに使います。
...
**ラベル用リアクション**（メッセージをカテゴリ付きでバッファに追加）
...
```yaml
reactions:
  pushpin: メモ           # :pushpin: が押されたら「メモ」として汎用収集
