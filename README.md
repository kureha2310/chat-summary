...
3. 名前（例：`chat-summary`）を入力してワークスペースを選択 → **Save**
4. 表示された **Internal Integration Token**（`secret_` で始まる文字列）をコピー
   → これが `NOTION_TOKEN` です

#### 3-2. 保存先のデータベースを作る

1. Notion でページを新規作成（タイトル例：「Slack まとめ」）
2. `/table` と入力して **Table** を選択 → データベースが作成される
3. タイトル列の名前が **Name** になっていることを確認
   （他の名前の場合は `src/notion.js` の `Name:` を変更してください）

#### 3-3. インテグレーションをデータベースに接続

1. 作成したデータベースページの右上 **「…」** メニューをクリック
2. **Connect to** → 作成したインテグレーション名を選択
